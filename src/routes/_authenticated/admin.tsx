...
function AdminPage() {
  ...
  const [riderKyc, setRiderKyc] = useState<any[]>([]);
  const [q, setQ] = useState("");
  const deleteUserFn = useServerFn(adminDeleteUser);

  const loadAll = async () => {
    const [{ data: rows }, { data: appsRows }, { data: riderRows }] = await Promise.all([
      supabase.from("profiles").select("*").order("created_at", { ascending: false }).limit(200),
      supabase.from("driver_applications").select("*").order("created_at", { ascending: false }),
      supabase.from("rider_verifications").select("*").order("created_at", { ascending: false }),
    ]);
    const list = rows ?? [];
  ...
  async function decideApp(id: string, status: "approved" | "rejected", notes?: string) {
    const { error } = await supabase.from("driver_applications").update({ status, admin_notes: notes ?? null }).eq("id", id);
    if (error) return toast.error(error.message);
    toast.success(`Driver application ${status}`);
    await loadAll();
  }

  async function decideRider(id: string, status: "approved" | "rejected", notes?: string) {
    const { error } = await supabase.from("rider_verifications").update({ status, admin_notes: notes ?? null }).eq("id", id);
    if (error) return toast.error(error.message);
    toast.success(`Rider ID ${status}`);
    await loadAll();
  }
  ...
  async function deleteUser(p: any) {
    if (!window.confirm(`Permanently delete ${p.name ?? "this user"}? This cannot be undone.`)) return;
    try {
  ...
  const pendingRider = riderKyc.filter((r) => r.status === "pending");
  const pendingDriver = apps.filter((a) => a.status === "pending");
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-5xl px-4 py-8">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-muted-foreground">
              {pendingRider.length} rider KYC · {pendingDriver.length} driver KYC pending review.
            </p>
          </div>
          <div className="relative w-64">
            <Search className="absolute left-2 top-2.5 h-4 w-4 text-muted-foreground" />
            <Input className="pl-8" placeholder="Search name, phone, employer…" value={q} onChange={(e) => setQ(e.target.value)} />
          </div>
        </div>

        {/* Rider ID verifications */}
        <h2 className="mb-3 text-2xl font-bold">Rider ID verifications</h2>
        <Card className="mb-10 divide-y">
          {riderKyc.length === 0 ? (
            <p className="p-6 text-center text-sm text-muted-foreground">No rider submissions yet.</p>
          ) : riderKyc.map((r) => (
            <div key={r.id} className="space-y-2 p-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  <p className="text-sm font-semibold">User <span className="font-mono text-xs">{r.user_id}</span></p>
                  <p className="text-xs text-muted-foreground">Submitted {new Date(r.created_at).toLocaleString()}</p>
                </div>
                <Badge variant={r.status === "approved" ? "default" : r.status === "rejected" ? "destructive" : "secondary"}
                  className={r.status === "approved" ? "bg-[color:var(--brand-green)] text-white" : ""}>
                  {r.status}
                </Badge>
              </div>
              <div className="grid gap-1 text-xs text-muted-foreground md:grid-cols-2">
                <div>ID number: {r.id_number ?? "—"}</div>
                <div>ID doc: {r.id_document_url ? <span className="font-mono">{r.id_document_url}</span> : "—"}</div>
                <div>Selfie: {r.selfie_url ? <span className="font-mono">{r.selfie_url}</span> : "—"}</div>
              </div>
              {r.admin_notes && <p className="text-xs"><b>Notes:</b> {r.admin_notes}</p>}
              {r.status === "pending" && (
                <div className="flex gap-2">
                  <Button size="sm" className="brand-gradient text-white" onClick={() => decideRider(r.id, "approved")}>Approve</Button>
                  <Button size="sm" variant="destructive" onClick={() => {
                    const notes = window.prompt("Rejection reason (optional):") ?? undefined;
                    decideRider(r.id, "rejected", notes);
                  }}>Reject</Button>
                </div>
              )}
            </div>
        ...
        {/* Driver applications */}
        <h2 className="mb-3 text-2xl font-bold">Driver applications</h2>
        <Card className="mb-10 divide-y">
          {apps.length === 0 ? (
            ...
            <div key={a.id} className="space-y-2 p-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  <p className="text-sm font-semibold">User <span className="font-mono text-xs">{a.user_id}</span></p>
              ...
              {a.status === "pending" && (
                <div className="flex gap-2">
                  <Button size="sm" className="brand-gradient text-white" onClick={() => decideApp(a.id, "approved")}>Approve & verify</Button>
                  <Button size="sm" variant="destructive" onClick={() => {
                    const notes = window.prompt("Rejection reason (optional):") ?? undefined;
        ...
        {/* User management */}
        <h2 className="mb-3 text-2xl font-bold">All users</h2>
            ...
            <div key={p.id} className="flex items-start gap-4 p-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Badge variant="outline" className="capitalize">{p.role ?? "rider"}</Badge>
              ...
              <div className="flex flex-col gap-2 sm:flex-row">
                <Button size="sm" variant={p.verified ? "outline" : "default"} className={p.verified ? "" : "brand-gradient text-white"} onClick={() => toggleVerified(p)}>
                  {p.verified ? <><ShieldOff className="mr-2 h-4 w-4" />Revoke</> : <><ShieldCheck className="mr-2 h-4 w-4" />Verify</>}
                </Button>
                <Button size="sm" variant="destructive" onClick={() => deleteUser(p)} disabled={p.id === user?.id}>
